...
func TestServer(t *testing.T) {
		...
		Repositories: []Repository {
			{
				...
				Alias: "gitdb-reference",
				Public: false,
			},
		},
	}
	jcb, err := json.Marshal(rc)
	...
	t.Run("test_zip", func(t *testing.T) {
		resp, err := http.Get(fmt.Sprintf("http://localhost:%d/zip/gitdb-reference/master/", sendPort))
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, resp.StatusCode)
