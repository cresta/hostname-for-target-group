...
SERVICE_NAME="hostname-for-target-group"
...
BUILDER_IMAGE=builder-${SERVICE_NAME}:${TAG}-builder
...
function build() {
  go mod download
  go mod verify
  GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o ${SERVICE_NAME} -ldflags '-extldflags "-f no-PIC -static"' -tags 'osusergo netgo static_build' ./cmd/${SERVICE_NAME}
}
